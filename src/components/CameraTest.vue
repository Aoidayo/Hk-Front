<template>
  <div class="container">
    <div id="divPlugin" class="plugin" style="width: 1000px;height: 700px;margin:0 auto"></div>
    <el-row style="margin:10px auto">
      <el-select style="margin:0 10px;max-height: 30px;" v-model="selectedChannel" :popper-append-to-body="true">
        <el-option v-for="channel in channels" :key="channel.id" :value="channel.id" :label="channel.name">
          {{ channel.name }}
        </el-option>
      </el-select>
      <el-button size="medium" type="danger" @click="showDVR()">连接录像机</el-button>
      ...
      <el-button size="medium" type="danger" @click="getAnalogChannelInfo">获取模拟通道信息</el-button>

      <el-button size="medium" type="danger" @click="getDigitalChannelInfo">获取数字通道信息</el-button>
...
<script>
import {DestIP, DestPort, DestIp_Port, LOGIN_PROTO, RTSP_PORT, USERNAME, PASSWORD} from "@/NVRconfig";
    ...
    showDVR() {
      WebVideoCtrl.I_InitPlugin({
        ...
        cbSelWnd: function (xmlDoc) {
          let g_iWndIndex = parseInt(xmlDoc.getElementsByTagName("SelectWnd")[0].textContent, 10);
        ...
        cbInitPluginComplete: async () => {
          try {
            ...
            // 登录设备
            await WebVideoCtrl.I_Login(DestIP, LOGIN_PROTO, DestPort, USERNAME, PASSWORD, {
              success: () => {
                console.log("登录成功");
                ...
                WebVideoCtrl.I_GetDigitalChannelInfo(DestIp_Port, {
                  success: (xmlDoc) => {
                    ...
                    const oChannels = xmlDoc.getElementsByTagName('InputProxyChannelStatus');
      ...
      WebVideoCtrl.I_InitPlugin({
        ...
        cbInitPluginComplete: async () => {
          try {
            ...
            // 登录设备
            await WebVideoCtrl.I_Login(DestIP, 1, DestPort, 'admin', 'cumtb617', {
              success: () => {
                console.log("登录成功");
              },
              error: () => {
            ...
            // 实时预览
            await WebVideoCtrl.I_StartRealPlay(DestIp_Port, {iWndIndex: 1, bZeroChannel: false, iPort: 554});
            console.log("预览成功");
